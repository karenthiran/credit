...
export async function PUT(
  ...
  try {
    ...
    if (body?.newLoan) {
      ...
      // ── NEW: also write the new Loan document ──────────────────────
      await Loan.updateOne(
        { loanId: newLoanId },
        {
          $setOnInsert: {
            loanId: newLoanId,
            customerId,
            customerName: body?.name ?? existingCustomer.name,
            loanAmount,
            interestRate,
            duration,
            totalWithInterest,
            ...
            paidAmount: 0,
            status: "ongoing",
            openedAt: resolvedLoanStartDate,
          },
        },
        { upsert: true },
      );

      // Mark the previous (now-completed) loan as completed in the Loan collection
      const prevLoanId = (existingCustomer as any).loanId;
      if (prevLoanId) {
        await Loan.findOneAndUpdate(
          { loanId: String(prevLoanId) },
          {
            $set: {
              status: "completed",
              closedAt: resolvedLoanStartDate,
              paidAmount: Number(existingCustomer.totalWithInterest || 0),
            },
          },
        );
      }
